...
fn process(input: &str) -> u32 {
    ...
    let result = dijkstra(
        &start,
        |&state| {
            let forward = if state.consecutive < 9 {
            ...
            // very hacky
            let available_directions = if (state.x == 0 && state.y == 0) || state.consecutive >= 3 {
                [
                    forward,
        ...
        |&p| p.consecutive >= 3 && p.x == grid.width as isize - 1 && p.y == grid.height as isize - 1,
    );
